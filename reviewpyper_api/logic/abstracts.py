...
def screen_abstracts(
    ...
    title_review_filter = TitleReviewFilter(
        title_review_path, abstracts_path=abstracts_txt_path_preprocessed, column_name=column_name
    )
    _, abstracts_txt_path_preprocessed_filtered = title_review_filter.run()

    abstract_screening = AbstractScreener(
        api_key_path=api_key_path,
        csv_path=abstracts_txt_path_preprocessed_filtered,
        question=question,
    ...
    post_processor = PostProcessing(
        file1_path=abstracts_txt_path_preprocessed,
        file2_path=screened_abstract_path,
        pubmed_csv_path=title_review_path,
    )
    post_processor.run()

    master_list_path = post_processor.final_file_path
    return {
        "screened_abstract_path": screened_abstract_path,
